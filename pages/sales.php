...
try {
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | TODAY'S SALES BY FOOD CATEGORY
    |--------------------------------------------------------------------------
    | Uses completed payments and the category assigned to each food item.
    | This keeps the dashboard category cards aligned with today's revenue.
    */
    $categoryStmt = $pdo->query("
        ...
        INNER JOIN categories c ON c.id = fm.category_id
    ...
    */
    ...
    if ($orderSearch !== '') {
        $orderWhere[] = '(
            ...
            OR EXISTS (
                SELECT 1
                FROM order_items osi_search
                INNER JOIN food_menu fm_search ON fm_search.id = osi_search.food_id
                LEFT JOIN categories c_search ON c_search.id = fm_search.category_id
                WHERE osi_search.order_id = o.id
                  AND (
                      fm_search.name LIKE :order_search_food
                      OR COALESCE(c_search.name, \'\') LIKE :order_search_category
                  )
            )
        )';
        $orderParams[':order_search'] = '%' . $orderSearch . '%';
        $orderParams[':order_search_id'] = '%' . $orderSearch . '%';
        $orderParams[':order_search_food'] = '%' . $orderSearch . '%';
        $orderParams[':order_search_category'] = '%' . $orderSearch . '%';
    }
...
<head>
    ...
    * {
    ...
    /* =========================================================
       CURRENT DASHBOARD — COMPACT CATEGORY KPIs
       ========================================================= */
    .metrics-compact {
    ...
    .metric-category-1 {
        background: linear-gradient(135deg, #21a66a 0%, #168652 100%) !important;
    }

    .metric-category-2 {
        background: linear-gradient(135deg, #7655b5 0%, #5d4096 100%) !important;
    }

    .metric-category-3 {
        background: linear-gradient(135deg, #4c7ed2 0%, #315eae 100%) !important;
    }

    .metric-category-4 {
        background: linear-gradient(135deg, #e96d15 0%, #c9550b 100%) !important;
    }

    .metric-orders {
        background: linear-gradient(135deg, #34495e 0%, #253545 100%) !important;
    }
        ...
        <main class="main">
            ...
            <div class="content">
                ...
                <section class="metrics metrics-compact">
                    ...
                    <article class="metric metric-category metric-category-<?= ($index % 4) + 1 ?>">
                        <div class="metric-top">
                        ...
                        <small><?= number_format((int)$category['quantity']) ?> items sold today</small>
                    </article>
                    <?php endforeach; ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="category-badges">
                                                <?php foreach (array_filter(array_map('trim', explode(',', (string)($recent['categories'] ?? '')))) as $catName): ?>
                                                <span
                                                    class="category-badge"><?= htmlspecialchars($catName, ENT_QUOTES, 'UTF-8') ?></span>
                                                <?php endforeach; ?>
                                            </span>
                                        </td>
